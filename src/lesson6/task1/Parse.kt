...
fun dateStrToDigit(str: String): String {
    val month = str.filter { !it.isDigit() }.trim()

    val digitalMonth = when (month) {
        "января" -> "01"
        "февраля" -> "02"
        "марта" -> "03"
        "апреля" -> "04"
    ...
    return str.replace(" $month ", ".${digitalMonth}.")
}
/**
 * Средняя (4 балла)
 *
...
fun mostExpensive(description: String): String {
    ...
    val foods = description.split(Regex("""(;? )"""))
    var mostExpensive: Pair<String, Double> = Pair(foods[0], foods[1].toDouble())
    val size = foods.size
    var index = 2
    while (index < size) {
        val current = foods[index + 1].toDouble()
        if (current > mostExpensive.second) mostExpensive = Pair(foods[index], current)
        index += 2
    }
